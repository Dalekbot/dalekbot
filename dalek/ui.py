...
def you_selected_challenge(current_challenge):
    if current_challenge == 1:
        ...
        debug.print_to_all_devices("You selected Straight-Line Speed Test", "StL")
    elif current_challenge == 3:
        debug.print_to_all_devices("You selected Minimal Maze", "MM")
    elif current_challenge == 4:
        debug.print_to_all_devices("You selected Somewhere Over The Rainbow", "OR")
...
def display_selected_challenge(current_challenge):
  ...
  debug.print_to_all_devices(colored("\n\n\n\n           Challenge Select \n", 'red'))
  if current_challenge == 1:
    debug.print_to_all_devices(colored("        >>> Obstacle Course <<<",'green'))
  else:
    debug.print_to_all_devices("           Obstacle Course")
  if current_challenge == 2:
    debug.print_to_all_devices(colored("        >>> Straight-Line Speed Test <<<",'green'))
  else:
    debug.print_to_all_devices("           Straight-Line Speed Test")
  if current_challenge == 3:
    debug.print_to_all_devices(colored("        >>> Minimal Maze <<<",'green'))
  else:
    debug.print_to_all_devices("           Minimal Maze")
  if current_challenge == 4:
    debug.print_to_all_devices(colored("        >>> Somewhere Over The Rainbow <<<",'green'))
  else:
  ...
  if current_challenge == 6:
    debug.print_to_all_devices(colored("        >>> Duck Shoot <<<",'green'))
  else:
    debug.print_to_all_devices("           Duck Shoot")
  if current_challenge == 7:
    debug.print_to_all_devices(colored("        >>> Slightly Deranged Golf <<<",'green'))
  else:
